...
function renderCritical(){
  ...
  criticalOrders.forEach(order=>{
    ...
    const stepsHtml = stageLabels.map((label, i)=>{
      ...
      const icon = i < order.stage ? '<i class="ph-bold ph-check"></i>' : stageIcons[i];
  ...
  recentOrders.forEach(order=>{
    ...
    if(order.status === 'received'){
      statusHtml = `
        ...
        <span><i class="ph ph-clock"></i> ${order.meta}</span>`;
    } else {
      statusHtml = `
        <span class="status-pill separating"><i class="ph ph-clipboard-text"></i> ${order.statusLabel}</span>
        <span><i class="ph ph-user"></i> Técnico: ${order.technician}</span>`;
    }
...
function openDetails(order){
  ...
  `;
  detailsOverlayEl.classList.add('show');
}
